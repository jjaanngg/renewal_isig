...
export default function HomePage(){
 ...
 useEffect(()=>{if(new URLSearchParams(window.location.search).get("preview")==="1"){setDocs([{share_slug:"preview-1",content:"# 개인화기 손질 및 점검",created_at:new Date().toISOString(),topic:"장비·점검"},{share_slug:"preview-2",content:"# 신병 교육 인수인계",created_at:new Date(Date.now()-86400000).toISOString(),topic:"교육·안내"},{share_slug:"preview-3",content:"# 월말 재고 정리",created_at:new Date(Date.now()-172800000).toISOString(),topic:"정기 업무"}]);setChecking(false);return;} supabase.auth.getUser().then(async({data})=>{if(!data.user){router.replace("/login");return;} setChecking(false); const {data:rows}=await supabase.from("documents").select("share_slug, content, created_at, topic").eq("user_id",data.user.id).order("created_at",{ascending:false}); setDocs(rows??[]);});},[router]);
